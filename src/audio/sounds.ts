// 効果音まわり（DESIGN §9）。外部音声ファイルは同梱しない方針なので、web ではブラウザ内蔵の
// Web Audio API でごく短い音を「合成」して鳴らす（外部素材ゼロ）。ぴよぴよさんすうと同方式。
//
...
// name ごとの音色。強い音を避け、やさしい音を意図（DESIGN §5 感覚にやさしく）。
...
// がんばりカードで⭐を1つずつ出すときの音。index が増えるほど半音ずつ高くする。
